...
class SdnMplsApp(app_manager.RyuApp) :
	...
	def judge(self, pkt_ipv4, info_list):
		src_ip = pkt_ipv4.src
		dst_ip = pkt_ipv4.dst
		...
		for dict in info_list:
			...
			if dict.get('src_ip')!= None and dict.get('dst_ip') == None:
			...
			#	dict.get('src_port') == None and dict.get('dst_port') == None:
			elif dict.get('src_ip')!= None and dict.get('dst_ip')!= None:
	...
	def process_topo_graph_and_paths(self) :
		...
		#get shorstest path
		self.spf_path_set = nx.all_pairs_dijkstra_path(self.topo_graph)
	...
	def handle_ping_ipv4(self, datapath, pkt, etherFrame, in_port) :
		...
		self.installMplsPath(forward_path, ip_src_ip, ip_dst_ip, ip_src_dp, ip_dst_dp, ip_src_port, ip_dst_port, pkt_ipv4)
	...
	# install MPLS path
	def installMplsPath(self, path, src_ip, dst_ip, begin_dp, end_dp, begin_in_port, end_out_port, pkt_ipv4) :
		...
		if path_size <= 0 :
			...
			field = dp.ofproto_parser.OFPMatchField.make(dp.ofproto.OXM_OF_MPLS_LABEL, int(label))
